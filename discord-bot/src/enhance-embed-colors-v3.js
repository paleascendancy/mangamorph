...
if (!source.includes('const EMBED_COLOR_PALETTE = [')) {
  ...
  if (!source.includes(marker)) throw new Error('Ponto de inserção da paleta não encontrado.');
  source = source.replace(marker, marker + paletteBlock);
  changed = true;
}

const paletteFunctions = String.raw`
...
function colorPaletteComponents(currentColor) {
  ...
  return [
    colorSelectRow(first, 'es_color_palette_1', 'Cores 1/2 • azul, verde, amarelo e vermelho', currentColor),
...
async function colorPalettePayload(interaction, session) {
  ...
  return {
    content:
      ...
      'Escolha uma cor nas listas abaixo. Para outra tonalidade, use **Cor personalizada**.',
...
if (!source.includes('function colorPaletteComponents(')) {
  ...
  if (!source.includes(marker)) throw new Error('authorModal não encontrado.');
  source = source.replace(marker, paletteFunctions + '\n' + marker);
  changed = true;
}

const selectLine = "const isSelect = interaction.isStringSelectMenu() && interaction.customId === 'es_select_embed';";
if (source.includes(selectLine)) {
...
`;
  if (!source.includes(channelBlock)) throw new Error('Bloco do seletor de canal não encontrado.');
  source = source.replace(channelBlock, channelBlock + colorBlock);
  changed = true;
}
...
const newColorHandler = [
  "      if (interaction.customId === 'es_color') { await interaction.update(await colorPalettePayload(interaction, session)); return true; }",
...
if (!source.includes("interaction.customId === 'es_color_custom'")) {
  throw new Error('Fluxo de paleta não foi aplicado.');
}
