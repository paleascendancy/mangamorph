...
document.querySelector("#relatedGrid").innerHTML = related.map(function(item,index) {
  ...
  return '<article class="related-card related-card-premium" data-related="' + item.id + '" tabindex="0" role="link" aria-label="Abrir ' + item.title + '">' +
    '<div class="related-cover" style="--accent:' + item.accent + '">' +
      '<span class="related-rank">#' + String(index+1).padStart(2,"0") + '</span>' +
      '<span class="related-open" aria-hidden="true">↗</span>' +
      '<strong class="related-cover-title">' + item.title + '</strong>' +
    '</div>' +
    '<div class="related-info">' +
      '<strong>' + item.title + '</strong>' +
      '<span>' + info.flag + ' ' + item.type + ' · ' + item.genre + '</span>' +
      '<small><span>Cap. ' + item.chapter + '</span><span>★ ' + item.rating.toFixed(1).replace(".",",") + '</span></small>' +
    '</div>' +
  '</article>';
}).join("");
...
const commentsKey = "mangamorph:comments:" + manga.id;
...
function renderComments() {
  const comments = loadComments().sort(function(a,b){ return b.createdAt - a.createdAt; });
  ...
  commentList.innerHTML = comments.map(function(comment) {
    return '<article class="comment-item" data-comment-id="' + comment.id + '">' +
      '<div class="comment-avatar small">C</div>' +
      '<div class="comment-body">' +
        '<div class="comment-topline"><strong>Convidado</strong><span>' + commentDate(comment.createdAt) + '</span></div>' +
        '<p>' + escapeHtml(comment.text) + '</p>' +
        '<div class="comment-actions">' +
          '<button type="button" data-like-comment="' + comment.id + '" class="' + (comment.liked ? 'active' : '') + '">♡ <span>' + (comment.likes || 0) + '</span></button>' +
          '<button type="button" data-delete-comment="' + comment.id + '">Excluir</button>' +
        '</div>' +
      '</div>' +
    '</article>';
  }).join("");
...
commentForm.addEventListener("submit", function(event) {
  ...
  renderComments();
  showToast("Comentário publicado neste dispositivo.");
});
...
commentList.addEventListener("click", function(event) {
  ...
  if (likeButton) {
    const id = likeButton.dataset.likeComment;
    const comment = comments.find(function(item){ return item.id === id; });
    if (!comment) return;
  ...
  if (deleteButton) {
    ...
    saveComments(comments.filter(function(item){ return item.id !== id; }));
    renderComments();
