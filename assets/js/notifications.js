...
async function openNotifications(){await ensureService();if(!db||!session){window.dispatchEvent(new CustomEvent("mangamorph:open-login"));return}await load();if(panel)panel.hidden=false;document.body.style.overflow="hidden"}
...
markAll?.addEventListener("click",async()=>{await ensureService();if(!db||!session)return;await db.from("mangamorph_notifications").update({read_at:new Date().toISOString()}).eq("user_id",session.user.id).is("read_at",null);await load()});
list?.addEventListener("click",async e=>{const row=e.target.closest("[data-notification-id]");if(!row)return;await ensureService();if(!db||!session)return;await db.from("mangamorph_notifications").update({read_at:new Date().toISOString()}).eq("id",Number(row.dataset.notificationId)).eq("user_id",session.user.id);const href=row.dataset.href;if(href)location.href=href;else{await load();close()}});

let servicePromise=null;
function ensureService(){
  if(servicePromise)return servicePromise;
  servicePromise=(async()=>{try{const {createClient}=await import("https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm");db=createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});db.auth.onAuthStateChange((event,next)=>{session=next;setTimeout(()=>{load();checkAdmin()},0)});const {data:{session:s}}=await db.auth.getSession();session=s;await Promise.all([load(),checkAdmin()])}catch(error){console.warn("MangaMorph notifications unavailable:",error);if(adminLink)adminLink.hidden=true}})();
  return servicePromise;
}
const start=()=>ensureService();
if("requestIdleCallback" in window)requestIdleCallback(start,{timeout:3000});else setTimeout(start,900);
