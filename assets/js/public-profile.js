...
async function load(){
  ...
  $("publicProfileCard").hidden=false;document.querySelector(".public-stats").hidden=false;document.querySelector(".public-section").hidden=false;$("publicError").hidden=true;
...
await load();
if(profileId){
  liveChannel=supabase.channel("mangamorph-public-profile-"+profileId)
...
window.addEventListener("beforeunload",()=>{if(liveChannel)try{supabase.removeChannel(liveChannel)}catch{}});
