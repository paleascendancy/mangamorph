...
export function SiteHeader({ isAuthenticated, role }: SiteHeaderProps) {
  ...
  return (
    <>
      ...
      <aside className={`profile-panel${isProfileOpen ? ' is-open' : ''}`} aria-hidden={!isProfileOpen}>
        ...
        <div className="guest-profile"><div className="guest-avatar" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="3.5" /><path d="M5 20c.8-4 3.2-6 7-6s6.2 2 7 6" /></svg></div><div><strong>{isAuthenticated ? 'Perfil' : 'Perfil convidado'}</strong><p>{isAuthenticated ? (role === 'admin' ? 'Administrador' : 'Conta MangaMorph') : 'Entre para salvar suas obras'}</p></div></div>
        {isAuthenticated ? (
          <div className="guest-auth-actions">
            {role === 'admin' && <a href="/admin" className="profile-primary-action">Admin</a>}
            <form action={signOut}><button type="submit" className="profile-secondary-action">Sair</button></form>
          </div>
        ) : (
